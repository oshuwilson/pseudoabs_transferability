#-------------------------------------------------------------------------------
# Spatial Bayesian Additive Regression Trees
#-------------------------------------------------------------------------------
...
for(z in 1:nrow(meta)){
  ...
  for(missing in missing_options){
    ...
    for(i in sites){
      ...
      bart_boyce$test_site <- i
      bart_boyce_final <- bind_rows(bart_boyce_final, bart_boyce)
    }
    ...
    if(missing == missing_options[1]){
      all_boyce <- bart_boyce_final
    } else {
      all_boyce <- rbind(all_boyce, bart_boyce_final)
    }
  ...
  #export boyce scores
  saveRDS(all_boyce, 
          file = paste0("output/spatial/bart/", this.species, "_", this.site, "_", this.stage, "_boyce_scores_bart.RDS"))
  
  # export metrics
  saveRDS(all_metrics,
          file = paste0("output/spatial/bart/", this.species, "_", this.site, "_", this.stage, "_metrics_bart.RDS"))
  
  # export self test
  saveRDS(all_self,
          file = paste0("output/spatial/bart/", this.species, "_", this.site, "_", this.stage, "_self_test_bart.RDS"))
