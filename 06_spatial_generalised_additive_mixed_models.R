#-------------------------------------------------------------------------------
# Spatial Generalised Additive Mixed Models
#-------------------------------------------------------------------------------
...
for(z in 5:nrow(meta)){
  ...
  rm(list=setdiff(ls(), c("meta", "meta2", "predictors", "z", "missing", "pred_gam")))
  ...
  for(missing in missing_options){
    ...
    for(i in sites){
      ...
      # predict
      test$buff_suitability <- predict.bam(buff_gam, test, type = "response", exclude = "individual_id")
      test$back_suitability <- predict.bam(back_gam, test, type = "response", exclude = "individual_id")
      test$crw_suitability <- predict.bam(crw_gam, test, type = "response", exclude = "individual_id")
      ...
      #boyce scores
      gamm_boyce <- expand.grid(buff = buff_boyce, back = back_boyce, crw = crw_boyce)
      gamm_boyce$test_site <- i
      gamm_boyce_final <- bind_rows(gamm_boyce_final, gamm_boyce)
    }
    
    # join together boyce scores
    if(missing == missing_options[1]){
      all_boyce <- gamm_boyce_final
    } else {
      all_boyce <- rbind(all_boyce, gamm_boyce_final)
    }
  ...
  #export boyce scores
  saveRDS(all_boyce, 
          file = paste0("output/spatial/gamm/", this.species, "_", this.site, "_", this.stage, "_boyce_scores_gamm.RDS"))
  
  # export self test
  saveRDS(all_self,
          file = paste0("output/spatial/gamm/", this.species, "_", this.site, "_", this.stage, "_self_test_gamm.RDS"))
