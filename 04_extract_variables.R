#script for extracting environmental covariates to tracks
...
min_date <- min(tracks$date) #important for wind and chlorophyll later on
...
if(min_date < cutoff_00){
  ...
  try(tracks_99$wind <- sqrt(tracks_99$wind_east^2 + tracks_99$wind_north^2))
