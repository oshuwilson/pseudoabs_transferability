...
for(z in 1:nrow(meta)){
  ...
  for(missing in missing_options){
    ...
    if(sum(is.na(buff)) < 0.1*nrow(buff) & sum(is.na(buff)) > 0){
      buff_mice <- miceRanger(buff, m=1)
      buff <- completeData(buff_mice)[[1]]
    }
    ...
    #check for NA - less than 10% of training data okay for imputing
    if(sum(is.na(back)) < 0.1*nrow(back) & sum(is.na(back)) > 0){
      back_mice <- miceRanger(back, m=1)
      back <- completeData(back_mice)[[1]]
    }
    ...
    #check for NA - less than 10% of training data okay for imputing
    if(sum(is.na(crw)) < 0.1*nrow(crw) & sum(is.na(crw)) > 0){
      crw_mice <- miceRanger(crw, m=1)
      crw <- completeData(crw_mice)[[1]]
    }
    ...
    if(missing == missing_options[1]){
      all_boyce <- rf_boyce_final
    } else {
      all_boyce <- rbind(all_boyce, rf_boyce_final)
    }
  ...
  #export boyce scores
  saveRDS(all_boyce, 
          file = paste0("output/spatial/rf/", this.species, "_", this.site, "_", this.stage, "_boyce_scores_rf.RDS"))
  
  # export metrics
  saveRDS(all_metrics,
          file = paste0("output/spatial/rf/", this.species, "_", this.site, "_", this.stage, "_metrics_rf.RDS"))
  
  # export self test
  saveRDS(all_self,
          file = paste0("output/spatial/rf/", this.species, "_", this.site, "_", this.stage, "_self_test_rf.RDS"))
