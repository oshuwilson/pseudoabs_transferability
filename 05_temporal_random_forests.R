...
for(z in c(18:21)){
  ...
  for(i in seasons){
    ...
    #define mtry values to vary over
    ...
    n_ind <- buff_sel %>% filter(pb == "presence") %>% pull(individual_id) %>% unique() %>% length()
    if(n_ind < 5){
      buff_v <- n_ind
    } else {
      buff_v <- 5
    ...
    #incorporate mtry values
    buff_params <- extract_parameter_set_dials(buff_wf) %>% #update model with these mtry values
      update(mtry = mtry(mtry_vals))
    
    #create tuning grid
    buff_grid <- grid_regular(buff_params)
    ...
    #set up model
    buff_best_mod <- rand_forest() %>%
      set_engine(engine = "ranger", importance = "impurity") %>%
      set_mode("classification") %>%
      set_args(trees = 1000, mtry = buff_best$mtry[1], min_n = 1)
    ...
    rm(buff_mice, buff_sel, buff_folds, buff_rec, buff_wf, buff_params, buff_grid, buff_tun, buff_best, buff_best_mod, buff_best_wf)
    ...
    #if number of individuals is fewer than 5, change v to n_ind
    n_ind <- back_sel %>% filter(pb == "presence") %>% pull(individual_id) %>% unique() %>% length()
    if(n_ind < 5){
      back_v <- n_ind
    } else {
      back_v <- 5
    ...
    #incorporate mtry values
    back_params <- extract_parameter_set_dials(back_wf) %>% #update model with these mtry values
      update(mtry = mtry(mtry_vals))
    
    #create tuning grid
    back_grid <- grid_regular(back_params)
    ...
    #set up model
    back_best_mod <- rand_forest() %>%
      set_engine(engine = "ranger", importance = "impurity") %>%
      set_mode("classification") %>%
      set_args(trees = 1000, mtry = back_best$mtry[1], min_n = 1)
    ...
    #remove unnecessary parameters to continue
    rm(back_mice, back_folds, back_rec, back_wf, back_params, back_grid, back_tun, back_best, back_best_mod, back_best_wf)
    ...
    #if number of individuals is fewer than 5, change v to n_ind
    n_ind <- crw_sel %>% filter(pb == "presence") %>% pull(individual_id) %>% unique() %>% length()
    if(n_ind < 5){
      crw_v <- n_ind
    } else {
      crw_v <- 5
    ...
    #incorporate mtry values
    crw_params <- extract_parameter_set_dials(crw_wf) %>% #update model with these mtry values
      update(mtry = mtry(mtry_vals))
    
    #create tuning grid
    crw_grid <- grid_regular(crw_params)
    ...
    #set up model
    crw_best_mod <- rand_forest() %>%
      set_engine(engine = "ranger", importance = "impurity") %>%
      set_mode("classification") %>%
      set_args(trees = 1000, mtry = crw_best$mtry[1], min_n = 1)
    ...
    rf_boyce <- expand.grid(buff = buff_boyce, back = back_boyce, crw = crw_boyce)
    rf_boyce$season <- i
    rf_boyce_final <- bind_rows(rf_boyce_final, rf_boyce)
  ...
  saveRDS(rf_boyce_final, 
          file = paste0("output/temporal/rf/", this.species, "_", this.site, "_", this.stage, "_boyce_scores_rf.RDS"))
  
  # export metrics
  saveRDS(all_metrics,
       file = paste0("output/temporal/rf/", this.species, "_", this.site, "_", this.stage, "_metrics_rf.RDS"))
  
  # export self test 
  saveRDS(all_self,
          file = paste0("output/temporal/rf/", this.species, "_", this.site, "_", this.stage, "_self_test_rf.RDS"))
